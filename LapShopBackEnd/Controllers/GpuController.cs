...
namespace LapShop.Api.Controllers
{
    ...
    public class GpuController : ControllerBase
    {
        ...
        public GpuController(IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var gpu = await _unitOfWork.Gpus.FindOneAsync(it => it.GpuId == id);
        ...
        public async Task<IActionResult> GetAll([FromHeader] int? skip, [FromHeader] int? take)
        {
            try
            {
                ...
                var paginatedGpus = await _unitOfWork.Gpus
                    .FindAsync(null, skipValue, takeValue, orderBy: cat => cat.GpuName,
                    orderByDirection: OrderBy.Ascending, null);


                if (paginatedGpus == null || paginatedGpus.Count() == 0)
                {
                    var errorResponse = new ApiResponse(null, ResponseStatus.NotFound)
                    {
                        Errors = new List<string> { "Gpus not found" }
                    };
                ...
                var response = new ApiResponse(paginatedGpus, ResponseStatus.Success);
                return Ok(response);
        ...
        [HttpPost]
        public async Task<IActionResult> AddNewGpu([FromBody] string gpuName)
        {
            ...
            try
            {
                ...
                {
                TbGpu newGpu = new TbGpu() { 
                GpuName=gpuName};
                   newGpu= await _unitOfWork.Gpus.AddAsync(newGpu);

                    await _unitOfWork.SaveAsync();


                    return Ok(new ApiResponse("New Gpu Id is : " + newGpu.GpuId, ResponseStatus.Success));
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateGpu([FromRoute] int id, [FromBody] string gpuName)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var existingGpu = await _unitOfWork.Gpus.FindOneAsync(it => it.GpuId == id);
                    ...
                    existingGpu.GpuName=gpuName;

                    _unitOfWork.Gpus.Update(existingGpu);

                    await _unitOfWork.SaveAsync();


                    return Ok(new ApiResponse(existingGpu, ResponseStatus.Success));
        ...
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteGpu([FromRoute] int id)
        {
            ...
            try
            {
                ...
                _unitOfWork.Gpus.Delete(existingGpu);


                await _unitOfWork.SaveAsync();


                return Ok(new ApiResponse($"Gpu with Id:[{id}] Was deleted succesfully", ResponseStatus.Success)); // Return 204 No Content
            }
            catch (Exception ex)
            {
